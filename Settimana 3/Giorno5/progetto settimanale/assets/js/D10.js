...
console.log(splitMe('Questa è una prova'));
...
let mail = stringa.replace(/[0-9]/g, '')
return mail
}
console.log(onlyLetters('33 Ciao a 4 tutti324'))
...
function isThisAnEmail(email) {
  ...
  return chiocciola !== -1 && punto !== -1 && punto > chiocciola;
}
console.log(isThisAnEmail('alessandropesole@pluto.com'));
...
function howManyDays (data) {
  ...
  let differenza = dataOdierna - data;
  giorniTrascorsi = Math.floor(differenza / (1000 * 60 * 60 * 24));
  return giorniTrascorsi
}
console.log(howManyDays(new Date ('2023-11-03')));
...
function isTodayMyBirthday () {
  let giornoCompleanno = 27;
  let meseCompleanno = 12;
  let giornoCorrente = new Date().getDate();
...
console.log('è oggi il mio compleanno:' + isTodayMyBirthday());
...
function newestMovie (array) {
    let riferimento = { Year: 1900 }
  array.forEach((movie) => {
    let annoFilm = parseInt(movie.Year)
    if (annoFilm > riferimento.Year) {
      riferimento = movie
    }
  })
...
function aggiungiElementoLista (testo) {
  ...
  appendi.appendChild(nuovoItem);
}
aggiungiElementoLista('Elemento')
/* ESERCIZIO 25
...
function isItPrime (numero) {
 for (let i = 2; i <= Math.sqrt(numero); i++) {
  if (numero % i === 0) {
    return 'il numero ' + numero + ' NON è un numero primo'
  }
 }
 return 'il numero ' + numero + ' è un numero primo'
